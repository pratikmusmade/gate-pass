...
PreparedStatement pstm = ConnectionProvider.getConnection()
			...
			<div class="container-fluid">
					...
					<div class="col">
						<div class="input-group mb-3 border border-secondary">
							<span class="input-group-text bg-dark text-light"
								...
								value="<%=rs.getString("request_date")%>" name="electionName"
								type="text" class="form-control bg-dark text-white"
								aria-label="Username" aria-describedby="basic-addon1">
						</div>
					</div>
				...
				<div class="row ">
					...
					<div class="col-lg-6 d-flex flex-row-reverse gap-3">
						...
						<button type="button" class="btn btn-success border border-2"
						onclick="acceptRequest(<%=rs.getString("request_id")%>)"
						>Accept
							Request</button>
						<button type="button" class="btn btn-danger border border-2"
						onclick="rejectRequest(<%=rs.getString("request_id")%>)"
						>Reject
							Request</button>
						<%
						}
						%>
						<button type="button" class="btn btn-primary border border-2"
